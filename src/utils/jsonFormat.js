...
export default function jsonObjToHTML(obj, jsonpFunctionName) {
  // if (process.browser) {
    // Constants
    let
      TYPE_STRING = 1,
...
// Core recursive DOM-building function
  ...
  let cachedJsonString = JSON.stringify(obj, null, 4);
  let jfPre=document.createRange().createContextualFragment('<pre id="jfContent_pre" />');
  return returnHTML;
}
